...
PATH_MONTHLY = 'L:\\NBRHC Security\\PROTECTIVE SERVICES SECURITY GUARDS\\SOCO LOG SHEET\\Template (DO NOT TOUCH)\\Old Temp\\'
PATH_MASTER = 'L:\\NBRHC Security\\PROTECTIVE SERVICES SECURITY GUARDS\\SOCO LOG SHEET\\Template (DO NOT TOUCH)\\Old Temp\\Backups\\Master DAR.xlsx'
PATH_MASTER_COPY = 'L:\\NBRHC Security\\PROTECTIVE SERVICES SECURITY GUARDS\\SOCO LOG SHEET\\Template (DO NOT TOUCH)\\Old Temp\\Backups\\Master DAR - USE THIS FILE FOR REVIEW.xlsx'
PATH_DRAFTS = 'L:\\NBRHC Security\\PROTECTIVE SERVICES SECURITY GUARDS\\SOCO LOG SHEET\\Template (DO NOT TOUCH)\\Old Temp\\Backups\\Draft Entries.xlsx'
PATH_LOGS = 'L:\\NBRHC Security\\REPORTS\\DAILY ACTIVITY REPORT\\DAR Submission Logs\\'
PATH_IMAGE = 'L:\\NBRHC Security\\PROTECTIVE SERVICES SECURITY GUARDS\\SOCO LOG SHEET\\Template (DO NOT TOUCH)\\Old Temp\\Backups\\security.ico'

# PATH_MONTHLY = 'L:\\Risk Services\\PATIENT EXPERIENCE\\Brendan\\DAR\\'
# PATH_MASTER = 'L:\\Risk Services\\PATIENT EXPERIENCE\\Brendan\\DAR\\Backups\\Master DAR.xlsx'
# PATH_MASTER_COPY = 'L:\\Risk Services\\PATIENT EXPERIENCE\\Brendan\\DAR\\Backups\\Master DAR - USE THIS FILE FOR REVIEW.xlsx'
# PATH_DRAFTS = 'L:\\Risk Services\\PATIENT EXPERIENCE\\Brendan\\DAR\\Backups\\Draft Entries.xlsx'
# PATH_LOGS = 'L:\\Risk Services\\PATIENT EXPERIENCE\\Brendan\\DAR\\Backups\\DAR Submission Logs\\' # 'U:\\Documents\\FILES\\PROJECTS\\DAR Application\\DAR Submission Logs\\' + str(datetime.now())[0:19].replace(':', '-') + ' DAR Entry.txt'
# PATH_IMAGE = 'L:\\Risk Services\PATIENT EXPERIENCE\\Brendan\\DAR\\Backups\\security.ico'
...
class App(tk.Tk):
    ...
    def __init__(self):
        ...
        self.service_call_type_list = ['NONE', 'Access', 'ACTT Alarms Sounding', 'Alarm', 'Arrest', 'Assist Police/EMS', 'Assist With ORB', 'By Law', 'Camera Audit',
                                       'Camera Footage Review', 'Camera Malfunction', 'Code Amber', 'Code Black', 'Code Blue/Pink', 'Code Brown', 'Code Green',
                                       'Code Grey', 'Code Orange', 'Code Purple', 'Code Red', 'Code Silver', 'Code White', 'Code Yellow', 'Daily Lock/Unlock',
                                       'ECS Testing', 'Elevator Kirkwood', 'Emergency Card Swipe Testing', 'Escort Delivery', 'Evidence/Contraband', 'Facility Maintenance',
                                       'Fall No Injuries', 'Fall Unknown Injuries', 'Fall With Injuries', 'GeoPro Alert', 'Guard Duties - Other', 'Helicopter Landing',
                                       'Helipad Check', 'Information', 'Investigate ECS Call', 'Lock/Unlock Door', 'Monitor Camera', 'Monitor Camera NDSAP',
                                       'Morgue Admission/Discharge', 'Morgue Viewings', 'Motor Vehicle Accident', 'Off-Site Checks', 'Off-Site Checks Cancelled',
                                       'Off-Site Service Calls', 'One-to-One', 'One-to-One Call-Outs', 'One-to-One Cover PSW Break', 'One-to-One Covered By PSW',
                                       'One-to-One Discontinued', 'One-to-One Request', 'Other Service Calls', 'Parking', 'Patient Belongings From Garage - ISO',
                                       'Patient Belongings to Garage - ISO', 'Patient Belongings - Locked Up', 'Patient Belongings - Retrieve', 'Patient Belongings - Other',
                                       'Patient Belongings - Tobacco Products', 'Patient Search', 'Patient - Security Presence/Assistance', 'Patient Transfer', 'Patrol Duties',
                                       'POI', 'Restraints', 'Search Room/Unit', 'Side Room Entry', 'Spider Alert', 'Staff Falls', 'Visitor - Forensics',
        ...
        self.months = {'1': '01 - January', '2': '02 - February', '3': '03 - March', '4': '04 - April', '5': '05 - May', '6': '06 - June', '7': '07 - July', '8': '08 - August', '9': '09 - September', '10': '10 - October', '11': '11 - November', '12': '12 - December' }
        ...
        self.winfo_toplevel().title('DAR Entry Tool')
    ...
    def get_dataframe(self):
        ...
        try:
            self.df = pd.read_excel(PATH_MONTHLY + CURRENT_YEAR + '\\' + MONTH_ENTERED + '\\DAR - ' + CURRENT_YEAR + ' ' + MONTH_ENTERED[5:] + '.xlsx')
    ...
    def get_master_dataframe(self):
        ...
        try:
            ...
            tk.messagebox.showinfo('Data Load Error', 'The Master DAR file can not be found. Please contact Jeremy or Brendan (x3063) to ensure that it has not been moved or renamed.')

    def get_saves_dataframe(self):
        '''Load the drafts dataframe. If it doesn't exist, create an empty one. Values aren't validate, so integers can come in as
           floats. Use the converters argument to read_excel to turn the integers into strings.'''

        try:
            self.saves_df = pd.read_excel(PATH_DRAFTS, converters={0:str, 2:str, 3:str, 4:str, 9:str, 10:str, 11:str})
    ...
    def handle_label_creation(self):
        ...
        self.shift_label = tk.Label(self, font=('Calibri', 12), text='SHIFT:').grid(row=0, column=0, pady=(10,0))
        self.date_label = tk.Label(self, font=('Calibri', 12), text='Date (mm/dd):').grid(row=2, column=0, sticky='e')
        self.call_received_time_label = tk.Label(self, font=('Calibri', 12), text='Call Received (24hr.):').grid(row=3, column=0, sticky='e')
        self.arrival_time_label = tk.Label(self, font=('Calibri', 12), text='Arrival Time (24hr.):').grid(row=4, column=0, sticky='e')
        self.completion_time_label = tk.Label(self, font=('Calibri', 12), text='Completion Time (24hr.):').grid(row=5, column=0, sticky='e', padx=(10,0))
        self.service_call_type_label = tk.Label(self, font=('Calibri', 12), text='Service Call Type:').grid(row=7, column=0, padx=(17,0))
        self.physical_intervention_label = tk.Label(self, font=('Calibri', 12), text='Physical Intervention:').grid(row=9, column=0, sticky='e')
        self.restraint_used_label = tk.Label(self, font=('Calibri', 12), text='Restraint Used:').grid(row=10, column=0, sticky='e')
        self.police_involved_label = tk.Label(self, font=('Calibri', 12), text='Police Involved:').grid(row=11, column=0, sticky='e')
        self.requested_by_label = tk.Label(self, font=('Calibri', 12), text='Requested By:').grid(row=12, column=0, sticky='e', pady=(15,0))
        self.contact_information_label = tk.Label(self, font=('Calibri', 12), text='Contact Information:').grid(row=13, column=0, sticky='e')
        self.notes_label = tk.Label(self, font=('Calibri', 14), text='Notes:').grid(columnspan=2, row=14, pady=(15,3))
        self.completion_time = tk.Label(self, font=('Calibri', 11), text='Was Time From Call to Completion >24 Hours?').grid(columnspan=2, row=17, column=0, sticky='w', padx=(20,0), pady=(5,3))
        ...
        self.h_line.grid(row=1, column=0, sticky='we', columnspan=2, pady=10, padx=12)
        self.h_line_2.grid(row=6, column=0, sticky='we', columnspan=2, pady=10, padx=12)
        self.h_line_3.grid(row=18, column=0, sticky='we', columnspan=2, pady=10, padx=12)
    ...
    def handle_radio_button_creation(self):
        ...
        self.date_morning_radio = tk.Radiobutton(self, text='7:30 - 19:30', padx=20, variable=self.var, value='7:30 - 19:30')
        self.date_morning_radio.grid(row=0, column=1, sticky='w', pady=(10,0))

        self.date_night_radio = tk.Radiobutton(self, text='19:30 - 7:30', padx=20, variable=self.var, value='19:30 - 7:30')
        self.date_night_radio.grid(row=0, column=1, padx=(110,0), pady=(10,0))
    ...
    def handle_entry_creation(self):
        ...
        self.call_received_entry.bind('<FocusOut>', self.focus_call_received_validation)
        self.call_received_entry.grid(row=3, column=1)

        self.arrival_time_entry = tk.Entry(self, width=5)
        self.arrival_time_entry.bind('<FocusOut>', self.focus_arrival_time_validation)
        self.arrival_time_entry.grid(row=4, column=1)

        self.completion_time_entry = tk.Entry(self, width=5)
        self.completion_time_entry.bind('<FocusOut>', self.focus_completion_time_validation)
    ...
    def handle_second_checkbox_creation(self):
        ...
        self.time_over_24_hours_checkbox.grid(columnspan=2, row=17, column=0, sticky='e', padx=(0,45), pady=(5,2))
        self.time_over_24_hours_checkbox.state(['!alternate'])

    def handle_textbox_creation(self):
        '''Create the textbox for Notes.'''

        self.notes_textbox = tk.Text(self, font=('Calibri', 9), height=3, width=60)
    ...
    def handle_button_creation(self):
        ...
        self.submit_button = tk.Button(self, font=('Arial', 16, 'bold'), text="SUBMIT", command=self.on_submit_button)
        self.submit_button.grid(columnspan=2, row=16, pady=10)

        self.save_button = tk.Button(self, font=('Arial', 11), fg='blue', text="Save Draft and Exit", command=self.on_save_button)
        self.save_button.grid(row=19, column=0, padx=(10,0), pady=(0,10), sticky='w')

        self.load_button = tk.Button(self, font=('Arial', 11), fg='blue', text="Load Saved Entry", command=self.on_load_button)
        self.load_button.grid(row=19, column=1, padx=(0,10),pady=(0,10), sticky='e')
    ...
    def handle_listbox_creation(self):
        ...
        self.service_call_type_entry = tk.Entry(self, textvariable=self.search_var, width=30)
        self.service_call_type_entry.grid(row=7, column=1, padx=(5,32)) # ENTRY WIDGET

        self.scrollbar = tk.Scrollbar(self, orient='vertical')
        self.scrollbar.grid(row=8, column=1, pady=10, sticky='nes')

        self.service_call_type_listbox = tk.Listbox(self, width=55, height=5, activestyle='none', yscrollcommand=self.scrollbar.set)
        self.service_call_type_listbox.bind('<FocusOut>', self.focus_service_call_type_validation)
        self.service_call_type_listbox.grid(columnspan=2, row=8, padx=10, pady=(10,15))

        self.scrollbar.config(command=self.service_call_type_listbox.yview) # move two up
    ...
    def update_list(self, *args):
        ...
        self.service_call_type_entry.config({'background': 'White'}) # Reset the text box
...
#########################################################################################################
    ...
    def focus_call_received_validation(self, event=None):
        ...
        try:
            if len(self.call_received_entry.get().strip()) < 4:
                fail = 1/0
            datetime.strptime(self.call_received_entry.get().strip().replace(':', ''), '%H%M')
    ...
    def focus_arrival_time_validation(self, event=None):
        ...
        try:
            if len(self.arrival_time_entry.get().strip()) < 4:
                fail = 1/0
            datetime.strptime(self.arrival_time_entry.get().strip().replace(':', ''), '%H%M')
    ...
    def focus_completion_time_validation(self, event=None):
        ...
        try:
            if len(self.completion_time_entry.get().strip()) < 4:
                fail = 1/0
            datetime.strptime(self.completion_time_entry.get().strip().replace(':', ''), '%H%M')
    ...
    def focus_service_call_type_validation(self, event=None):
        ...
        if self.service_call_type_listbox.get('active').strip() in self.service_call_type_list:
            temp_string = self.service_call_type_listbox.get('active').strip() # Because when you delete tray_entry text, the 'anchor' becomes ''
            self.service_call_type_entry.delete(0, 'end') # This resets the 'anchor' text
...
#########################################################################################################
    ...
    def date_validation(self, event=None):
        ...
        try:
            datetime.strptime(self.date_entry.get().strip(), '%m/%d')
            entered_date = datetime.strptime(self.date_entry.get().strip(), '%m/%d') # Make sure the date is in the proper format
...
##########################################################################################################
    ...
    def get_checkbox_answers(self):
        ...
        self.physical_intervention_answer = 'No'; self.restraint_used_answer = 'No'; self.police_involved_answer = 'No'; self.time_over_24_hours_answer = 'No'

        try:
            if (self.physical_intervention_checkbox.state()[0] == 'selected' or self.physical_intervention_checkbox.state()[1] == 'selected'): # Output will be one of: (), ('focus'), ('selected'), or ('focus', 'selected')
    ...
    def get_time_difference(self, time_1, time_2, check_24=None):
        ...
        if seconds/60 > 59:
            minutes = str(round((seconds/60/60 - hours) * 60 )) # Minutes past the floored hour

            if ((self.time_over_24_hours_answer == 'Yes') and (check_24 == 'Yes')): # check_24: time from call to arrival isn't increased by 24
                hours = hours + 24
    ...
    def append_row_to_df(self):
        ...
        self.master_df = self.master_df.append(self.row_to_append, ignore_index=True)
        # This line nor any following lines/functions will run if the Master DAR is not found
    ...
    def append_row_to_saves_df(self):
        ...
        (self.row_to_append_saves_test['Contact Information'] == '') and (self.row_to_append_saves_test['Notes'] == '')):
            ...
            tk.messagebox.showinfo('Nothing to Save', 'To save a draft, you must have entered data.')
            return True

        else:

            self.identifier_answer = simpledialog.askstring('Identifying Information', 'Please enter a J# or patient name:')
            ...
            if self.identifier_answer is not None:
                ...
                if self.saves_df.shape[0] == 10:
                    self.saves_df.drop(0, axis=0, inplace=True) # Drop the oldest draft

                self.saves_df = self.saves_df.append(self.row_to_append_saves, ignore_index=True) # Use index to pull in later?
    ...
    def save_text_file(self):
        ...
        try:
            with open(PATH_LOGS + str(datetime.now())[0:19].replace(':', '-') + ' DAR Entry.txt', 'w+') as file:

                file.write('\n  Below is the information for the record submitted on: ' + str(datetime.now())[0:19] + '\n')

                file.write('\n  Date: '.ljust(28) + self.row_to_append['Date'])
                file.write('\n  Shift: '.ljust(28) + self.row_to_append['Shift'])
                file.write('\n  Call Received Time: '.ljust(28) + self.row_to_append['Call Received Time'])
                file.write('\n  Arrival Time: '.ljust(28) + self.row_to_append['Arrival Time'])
                file.write('\n  Completion Time: '.ljust(28) + self.row_to_append['Completion Time'])
                file.write('\n  Service Call Type: '.ljust(28) + self.row_to_append['Service Call Type'])
                file.write('\n  Physical Intervention: '.ljust(28) + self.row_to_append['Physical Intervention'])
                file.write('\n  Restraint Used: '.ljust(28) + self.row_to_append['Restraint Used'])
                file.write('\n  Police Involved: '.ljust(28) + self.row_to_append['Police Involved'])
                file.write('\n  Requested By: '.ljust(28) + self.row_to_append['Requested By'])
                file.write('\n  Contact Information: '.ljust(28) + self.row_to_append['Contact Information'])
    ...
    def save_files(self):
        ...
        workbook.save(PATH_MONTHLY + CURRENT_YEAR + '\\' + MONTH_ENTERED + '\\DAR - ' + CURRENT_YEAR + ' ' + MONTH_ENTERED[5:] + '.xlsx')

        # MASTER DAR ========================================================================================================
        workbook_master = Workbook()
        worksheet_master = workbook_master.worksheets[0]
        worksheet_master.title = 'Master DAR'
        ...
        for row_index, row in enumerate(master_rows, 1):
            for column_index, value in enumerate(row, 1):
                worksheet_master.cell(row=row_index, column=column_index, value=value).alignment = Alignment(horizontal='center', vertical='center', wrapText=True)
    ...
    def reset_checkboxes(self):
        '''Uncheck all Checkboxes. I believe there is also a 'focused' parameter.'''
    ...
    def on_submit_button(self):
        ...
        if sum(self.errors.values()) == 0:
            ...
            if self.draft_opened_index is not None:
                self.saves_df.drop(self.draft_opened_index, axis=0, inplace=True) # Drop the draft from its dataframe
    ...
    def handle_topbox_listbox_creation(self):
        ...
        header_string = '      ' + 'IDENTIFIER'.ljust(15) + 'DATE'.ljust(9) + 'SHIFT'.ljust(15) + 'CALL RECEIVED'.ljust(15)
        header_string+= 'ARRIVAL'.ljust(10) + 'SERVICE CALL TYPE'.ljust(20)
        header_string+= 'PHYS. INTERV.'.ljust(17) + 'RESTRAINT USED'.ljust(17) + 'POLICE INVOLVED'.ljust(17)
        header_string+= 'REQUESTED BY'.ljust(15) + 'CONTACT INFO.'

        tk.Label(self.window, font=('Courier', 8), fg='blue', text=header_string).grid(row=1, padx=(0,4))

        self.top_lbox = tk.Listbox(self.window, font=('Courier', 8), width=170, height=10, activestyle='none')#, yscrollcommand=self.frame_scrollbar.set)
...
#         tray_name_longest_length = self.get_df_longest_tray_name_length()
        ...
        for row in range(self.saves_df.shape[0]):
            row_string = ('(' + str(index_number) + ')').rjust(4); index_number+= 1
            row_string+= '  ' + str(self.saves_df.iloc[row,0])[:12].ljust(15) # Identifying Information
            row_string+= str(self.saves_df.iloc[row,1])[:5].replace('nan', '').ljust(9) # Date
            row_string+= self.saves_df.iloc[row,2].ljust(15) # Shift
            row_string+= str(self.saves_df.iloc[row,3])[:10].replace('nan', '').ljust(15) # Call Received Time          USING STR() PREVENTS INTEGERS (11:00) FROM SHOWING AS FLOATS WITH DECIMALS IN THE LISTBOX
            row_string+= str(self.saves_df.iloc[row,4])[:7].replace('nan', '').ljust(10) # Arrival Time
            row_string+= str(self.saves_df.iloc[row,6])[:17].replace('nan', '').ljust(20) # Service Call Type
            row_string+= self.saves_df.iloc[row,7].ljust(17) # Physical Intervention
            row_string+= self.saves_df.iloc[row,8].ljust(17) # Restraint Used
            row_string+= self.saves_df.iloc[row,9].ljust(17) # Police Involved
            row_string+= str(self.saves_df.iloc[row,10])[:10].replace('nan', '').ljust(15) # Requested By
            row_string+= str(self.saves_df.iloc[row,11])[:13].replace('nan', '') # Contact Information
...
#             row_string+= self.saves_df.iloc[row,12].ljust(8) # Time Over 24 Hours
    ...
    def load_selected_draft(self):
        ...
        self.row_to_insert = [str(x).replace('nan', '') for x in self.row_to_insert]

        self.date_entry.insert('end', str(self.row_to_insert[1]))
        self.call_received_entry.insert('end', str(self.row_to_insert[3]).replace('.0', ''))
        self.arrival_time_entry.insert('end', str(self.row_to_insert[4]).replace('.0', ''))
        self.completion_time_entry.insert('end', str(self.row_to_insert[5]).replace('.0', ''))
        ...
        self.contact_information_entry.insert('end', str(self.row_to_insert[11]).replace('.0', ''))
    ...
    def on_select_button(self):
        ...
        try:
            ...
            self.draft_opened_index = row_selected # If submitted, this index will locate the draft to delete
            ...
            tk.messagebox.showinfo('Selection Error', 'Please select a draft.', parent=self.window)
    ...
    def on_load_button(self):
        ...
        try:
            ...
            if self.window is None or not self.window.winfo_exists():
                ...
                self.select_button = tk.Button(self.window, font=('Arial', 11, 'bold'), fg='blue', text='CLICK HERE to Import Selected Draft', command=self.on_select_button)
                self.select_button.grid(row=0, padx=(3,0), pady=10, sticky='we')
